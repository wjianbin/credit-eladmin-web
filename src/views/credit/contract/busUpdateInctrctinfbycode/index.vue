      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="uploadstatus" label="上报状态">
          <template slot-scope="scope">
            {{ dict.label.uploadstatus[scope.row.uploadstatus] }}
          </template>
        </el-table-column>
        ...
        <el-table-column v-permission="['admin','busUpdateInctrctinfbycode:edit','busUpdateInctrctinfbycode:del']" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import crudBusUpdateInctrctinfbycode from '@/api/contract/busUpdateInctrctinfbycode'
...
const defaultForm = { id: null, accloanid: null, infrectype: null, modreccode: null, rptdate: null, mdfcsgmtcode: null, uploadstatus: null, uploadflag: null, createTime: null }
export default {
  name: 'BusUpdateInctrctinfbycode',
  components: { pagination, crudOperation, rrOperation, udOperation },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  dicts: ['uploadstatus'],
  cruds() {
    return CRUD({ title: '/api/update_inctrctinfbycode', url: 'api/busUpdateInctrctinfbycode', sort: 'id,desc', crudMethod: { ...crudBusUpdateInctrctinfbycode }})
  },
  data() {
    return {
      permission: {
        add: ['admin', 'busUpdateInctrctinfbycode:add'],
        edit: ['admin', 'busUpdateInctrctinfbycode:edit'],
        del: ['admin', 'busUpdateInctrctinfbycode:del']
      },
      rules: {
        id: [
